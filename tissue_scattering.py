...
def detectorPhotons(x, y, z, amp, dDet, width):
    ...
    return photonAmplitude


def updatePositions(x, y, z, idx, amp, numPhotons, mu, absCoef):
...
def scatter(ld, dDet1, dDet2, width, dSkin, dMuscle, dBone, concBlood, ratioOxygen):
    ...
    while keepScattering:
        ...
        if numInSkin > 0:
            xPos, yPos, zPos, amplitude = updatePositions(xPos, yPos, zPos, idxSkin, amplitude, numInSkin, muSkin, 0)

        # we assume that the vast majority of blood is in the muscle not the skin or bone, hence the abs coefficient is 0
        if numInMuscle > 0:
            xPos, yPos, zPos, amplitude = updatePositions(xPos, yPos, zPos, idxMuscle, amplitude, numInMuscle, muMuscle,
                                                          absCoef)
            # print (temp, numInBone)
        if numInBone > 0:
            xPos, yPos, zPos, amplitude = updatePositions(xPos, yPos, zPos, idxBone, amplitude, numInBone, muBone, 0)
        ...
        numDet1Photons += detectorPhotons(xPos, yPos, zPos, amplitude, dDet1, width)
        numDet2Photons += detectorPhotons(xPos, yPos, zPos, amplitude, dDet2, width)
    ...
    return numDet1Photons, numDet2Photons
...
def scatteringSpectrum(ldStart, ldEnd, numSteps, conc, ratio):
    ...
    for idx in range(max(ldArray.shape)):
        # print (ldArray[idx])
        ampDet1[idx], ampDet2[idx] = scatter(ldArray[idx], 0.1, 0.3, 0.05, 0.05, 1, 3, conc, ratio)
    return ldArray, ampDet1, ampDet2
...
def scatteringOxygenRatioVariation(ratioStart, ratioEnd, numSteps, ld, conc):
    ...
    for idx in range(max(ratioArray.shape)):
        ...
        ampDet1[idx], ampDet2[idx] = scatter(ld, 0.1, 0.3, 0.05, 0.05, 1, 3, conc, ratioArray[idx])
    return ratioArray, ampDet1, ampDet2
...
def scatteringBloodConcentrationVariation(concStart, concEnd, numSteps, ld, ratio):
    ...
    for idx in range(max(concArray.shape)):
        # print (concArray[idx])
        ampDet1[idx], ampDet2[idx] = scatter(ld, 0.1, 0.3, 0.05, 0.05, 1, 3, concArray[idx], ratio)
    return concArray, ampDet1, ampDet2


def scanAllParameters(ldStart, ldEnd, numldSteps, concStart, concEnd, numConcSteps, ratioStart, ratioEnd,
                      numRatioSteps):
    ldArray = np.linspace(ldStart, ldEnd, numldSteps)
    concArray = np.linspace(concStart, concEnd, numConcSteps)
    data = np.zeros((numldSteps, numConcSteps, numRatioSteps, 5))
    for ldIdx in range(numldSteps):
        for concIdx in range(numConcSteps):
            ...
            cArr, d1Arr, d2Arr = scatteringOxygenRatioVariation(ratioStart, ratioEnd, numRatioSteps, ldArray[ldIdx],
                                                                concArray[concIdx])
...
# print (scanAllParameters(400, 900, 3, 0.125, 12.5, 3, 0, 1, 3))
